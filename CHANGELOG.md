...
- Invalid OpenAI tool-call argument markers stay provider-internal: recorded `llm.responded.tool_calls` entries serialize through `ToolCall.to_dict()` as `{id, name, args}` only; `LLMResponse.to_dict()` filters the private metadata marker; cache and recorded-fixture hydration ignore any persisted marker field; invalid-tool-input turns are skipped for durable LLM replay; runtime rejection still uses the existing `tool.invalid_input` failure path.
...
### CLI
- `activegraph fork --set <pack>.<key>=<value>` ships end-to-end per CONTRACT v1.1 #1 and frame `t3-implement-cli-set-flag`. The override is validated PRE-event against the pack's Pydantic `settings_schema` (D-2), coerced through field validators to the typed value (D-4), and persisted as a `fork.override.applied` event with the full-attestation payload — pack identity, key, typed value, and a schema-constraint snapshot for replay self-sufficiency across pack drift (D-3). Multiple `--set` flags compose; each produces one event. `activegraph replay --json` surfaces the projected `effective_settings`. The replay projector reads from the event log only — same-value re-sets are idempotent, different-value re-sets raise `ReplayDivergenceError` per §v0.5 #7 (D-1 "log is truth"). The T2 drift-gate allowlist entry for `--set` is removed in the same commit; `--memo` / `--search` remain pending and stay allowlisted.

  New helper module `activegraph.core.overrides` exports `validate_override`, `CoercedOverride`, and `InvalidOverrideError` for callers that want the pre-event validation surface without going through the CLI. `SQLiteEventStore(path)` now constructs without a `run_id` for file-level forensic reads (`store.load(run_id)`, `store.runs()`, `store.append(run_id=, event_type=, payload=)`), backward-compatible with the per-run protocol when both args are passed. `Runtime.load(run_id, store=...)` accepts a pre-opened store for the same forensic-reattach path. The diligence pack gains a `support_threshold` setting alongside `confidence_threshold_for_review` to give the override surface a second coercion target.
